...
#include "class/constant_medium.hpp"
...
void cornell_smoke()
{
    ...
    world.add(make_shared<constant_medium>(box1, 0.01, color(0, 0, 0), seed));
    world.add(make_shared<constant_medium>(box2, 0.01, color(1, 1, 1), seed));
...
void final_scene(int image_width, int samples_per_pixel, int max_depth)
{
    ...
    for (int i = 0; i < boxes_per_side; i++)
    {
        for (int j = 0; j < boxes_per_side; j++)
        {
            ...
            auto y0 = 0.0;
            auto x1 = x0 + w;
            auto y1 = random_double(1, 101, seed);
            auto z1 = z0 + w;
    ...
    world.add(boundary);
    world.add(make_shared<constant_medium>(boundary, 0.2, color(0.2, 0.4, 0.9), seed));
    boundary = make_shared<sphere>(point3(0, 0, 0), 5000, make_shared<dielectric>(1.5));
    world.add(make_shared<constant_medium>(boundary, .0001, color(1, 1, 1), seed));

    auto emat = make_shared<lambertian>(make_shared<image_texture>("earthmap.jpg"));
    world.add(make_shared<sphere>(point3(400, 200, 400), 100, emat));
    auto pertext = make_shared<noise_texture>(seed, 0.2);
    ...
    for (int j = 0; j < ns; j++)
    {
        boxes2.add(make_shared<sphere>(point3::random(0, 165, seed), 10, white));
    }
